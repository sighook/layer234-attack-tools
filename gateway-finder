#!/usr/bin/env python3
# Description: Identify routers on the local LAN and paths to the Net
# Depends on:  scapy
#
...
parser = OptionParser(usage="""%prog [ -I interface ] -i ip -f macs.txt

Tries to find a layer-3 gateway to the Internet.
Attempts to reach an IP address using ICMP ping and TCP SYN to port 80
via each potential gateway in macs.txt (arp-scan -l | tee macs.txt)
""")
parser.add_option("-i", "--ip",
        dest="ip",
        help="Internet IP to probe")
...
version = "1.1"
print(f"""
gateway-finder v{version} http://pentestmonkey.net/tools/gateway-finder

[+] Using interface {options.interface} (-I to change)
""")
...
ipofmac = {}
for line in lines:
    m = re.search('((?:(?:[a-fA-F0-9]){2}:){5}[a-fA-F0-9]{2})', line)
    if m and m.group(1):
        ipofmac[m.group(1).upper()] = "UnknownIP"
        m = re.search('((?:\d{1,3}\.){3}\d{1,3}).*?((?:[a-fA-F0-9]{2}:){5}[a-fA-F0-9]{2})', line)
        if m and m.group(1) and m.group(2):
            ipofmac[m.group(2).upper()] = m.group(1)
        else:
            m = re.search('((?:[a-fA-F0-9]{2}:){5}[a-fA-F0-9]{2}).*?((?:\d{1,3}\.){3}\d{1,3})', line)
            if m and m.group(1) and m.group(2):
...
for mac in macs:
    # Echo request, TTL=1
    packets.append({
        'packet': Ether(dst=mac)/IP(dst=options.ip,ttl=1)/ICMP(seq=seq),
        'type': 'ping',
        'dstip': options.ip,
        'dstmac': mac,
        'seq': seq,
        'message': """
%s [%s] appears to route ICMP Ping packets to %s.
Received ICMP TTL Exceeded in transit response.""" % (mac, ipofmac[mac], options.ip)
    })
    seq = seq + 1

    # TCP SYN to port 80, TTL=1
    packets.append({
        'packet': Ether(dst=mac)/IP(dst=options.ip,ttl=1)/TCP(seq=seq),
        'type': 'tcpsyn',
        'dstip': options.ip,
        'dstmac': mac,
        'seq': seq,
        'message': """
%s [%s] appears to route TCP packets %s:80.
Received ICMP TTL Exceeded in transit response.""" % (mac, ipofmac[mac], options.ip)
    })
    seq = seq + 1
    ...
    # Echo request
    packets.append({
        'packet': Ether(dst=mac)/IP(dst=options.ip)/ICMP(seq=seq),
        'type': 'ping',
        'dstip': options.ip,
        'dstmac': mac,
        'seq': seq,
        'message': """
We can ping %s via %s [%s]""" % (options.ip, mac, ipofmac[mac])
    })
    seq = seq + 1

    # TCP SYN to port 80
    packets.append({
        'packet': Ether(dst=mac)/IP(dst=options.ip)/TCP(seq=seq),
        'type': 'tcpsyn',
        'dstip': options.ip,
        'dstmac': mac,
        'seq': seq,
        'message': """
We can reach TCP port 80 on %s via %s [%s]""" % (options.ip, mac, ipofmac[mac])
    })
    seq = seq + 1
...
# vim:sw=4:ts=4:sts=4:et:tw=79:cc=80
# End of file.
